...
- **Map** — every stop on the route plotted on an OpenStreetMap map (via [Leaflet](https://leafletjs.com), free, no key needed), connected in order with arrows showing direction (outbound and return in different colours). Places are looked up automatically; "Update map" retries any that failed. Locations are cached on the trip itself, so it's instant after the first lookup, including for whoever you're syncing with.
